                                ...
                                <div key={day.toISOString()} className="border-r last:border-r-0 p-1 relative">
                                    {dayApps.map(app => (
                                        <button
                                            ...
                                            className={cn(
                                                "w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring text-xs p-1 rounded mb-1 border-l-4 truncate cursor-pointer hover:opacity-80 transition-opacity",
                                            ...
                                            aria-label={`Szczegóły wizyty: ${app.patient.lastName}, ${format(app.startDateTime, 'HH:mm')}${app.isPaid ? ' - Opłacona' : ''}`}
                                        >
                                            <div className="font-bold flex justify-between">
                                                <span>{format(app.startDateTime, 'HH:mm')}</span>
                                                {app.isPaid && <span className="text-[10px] text-green-700 font-extrabold" aria-hidden="true">$</span>}
                                            </div>
